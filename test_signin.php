<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>LRG</title>
    <link rel="stylesheet" href="/css/main.css">
</head>
<body>
<main id="app">
    <h1 class="hidden">London Referees Group</h1>
    <div class="main-container">
        <header class="main-header" id="mainHeader">
            <div class="top-banner">
                <p id="banner1">London Referees Group</p>
                <p id="banner2">Proudly Canadian</p>
            </div>
            <nav id="main-nav">
              <h2 class="hidden">Main Nav</h2>
                <img src="images/logo.svg" alt="mobile logo" id="mobile-logo">
                <button id="button"></button>
                    <div id="burger-con">
                        <ul id="burger-menu">
                            <li class="nav-item1"><a href="/">HOME</a></li>
                            <li class="nav-item2"><a href="#the-referee">THE REFEREE</a></li>
                            <li class="nav-item3"><a href="/partners">PARTNERS</a></li>
                            <li class="nav-item4">                    
                                <a href="/"><img src="images/logo.svg" alt="logo" id="main-logo"></a>
                            </li>
                            <li class="nav-item5"><a href="/membership">MEMBERSHIP</a></li>
                            <li class="nav-item6"><a href="/join">JOIN US</a></li>
                            <li class="nav-item7"><a href="/contact">CONTACT</a></li>
                        </ul>
                        <div id="burger-legal">
                            <p>Copyright © 2021</p>
                            <p>London Referees Group</p>
                            <p>Proudly Canadian</p>
                        </div>
                  </div>
            </nav>
        </header>


        <section class="test-body">
            <h1>This is for testing the functionality of Administration</h1>
            <h2>If successful, we'll apply these to other pages.</h2><br>

            <a href="admin/admin_login.php">Sign In</a><br>
            <a href="admin/admin_register.php">Sign Up</a>
        </section>

        <footer>
            <h2 class="hidden">Footer</h2>
            <div id="footer">
              <img src="images/footer_logo.svg" alt="footer logo" id="footer-logo"> 
              <ul id="footer-nav">
                  <li><a href="/#the-referee">The Referee</a></li>
                  <li><a href="/partners">Partners</a></li>
                  <li><a href="/membership">Membership</a></li>
                  <li><a href="/join">Join Us</a></li> 
                  <li><a href="/contact">Contact Us</a></li>       
              </ul>
              <ul id="footer-info">
                  <li><a href="">Bylaws</a></li>
                  <li><a href="">COVID-19</a></li>
                  <li><a href="">Supervisors</a></li>
                  <li><a href="">Report a Crime Offence</a></li>
                  <li><a href="">Match Penalty Reports</a></li>
                  <li><a href="">Brand Guidelines</a></li>
              </ul>
              <ul id="copyright">
                  <li>Copyright © 2021</li>
                  <li>London Referee Group</li>
                  <li>Proudly Canadian</li>
              </ul>
            </div>
        </footer>

  </div><!--end main-->
</main>
<script src="js/burger.js"></script>
<script src="js/scroll.js"></script>
</body>
</html>